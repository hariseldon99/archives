...
export ORDER=1 #Resonance order J_n(eta)=0
...
#Actual run command.
export RUNPROG="./heff_res_parallel"
...
do
  ...
  case "$VARYING_PARAM" in
    ...
    *)
    echo 
    echo "Error! Please edit the script and select the varying parameter: 'ampl' or 'alpha' ..."
    echo
    exit
    ;;
  esac
  ...
  mpirun -np $MPI_NUM_PROCS --bynode $RUNPROG $VERBOSITY -a $AMPLITUDE -e $ORDER -p $PERT_AMP -t $FINALTIME -n $TIMESTEPS -m "${OUTDIR}/${MAG_FILE}" 2>$ERROR_FILE
done
