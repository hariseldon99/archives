		     ...
		     void *params)
{
  ...
  //anisotropy term for delta
  delta_real = pk * delta_real;
  delta_imag = pk * delta_imag;
...
//gets the time average of magnitude
double
get_magavg (GArray * a)
{
  double avg = 0.0;
  ...
  while (stride < datasize)
    {
	  stride++;
	  real = ((double *) (void *) a->data)[stride];
	  stride++;
	  imag = ((double *) (void *) a->data)[stride];
	  stride++;
      avg = avg + sqrt(real*real + imag*imag);

    }
  avg = 2.0 * avg / datasize;
  return avg;
}
//
