...
{
  double y_re[STATENO], y_im[STATENO], dydt_re[STATENO], dydt_im[STATENO];
  double hamilt_mat[STATENO * STATENO];
  int nt, mt;

  /*Build the hamiltonian matrix elements */

  for (nt = 0; nt < STATENO; nt++)
    for (mt = 0; mt <= nt; mt++)
      {
	hamilt_mat[nt + STATENO * mt] = hamilt (param, nt, mt, t);
	hamilt_mat[mt + STATENO * nt] = hamilt_mat[nt + STATENO * mt];	//Always hermitian
      }
...

  {
	...
	dydt_re[nt] = 0.0;
	dydt_im[nt] = 0.0;
	for (mt = 0; mt < STATENO; mt++)
	  {
	    dydt_re[nt] =
	      dydt_re[nt] + hamilt_mat[nt + STATENO * mt] * y_im[mt];
	    dydt_im[nt] =
	      dydt_im[nt] - hamilt_mat[nt + STATENO * mt] * y_re[mt];
	  }
      }
  }
...
jac (double t, const double y[], double *dfdy, double dfdt[], void *param)
{
  ...
  gsl_matrix_view dfdy_mat =
    gsl_matrix_view_array (dfdy, 2 * STATENO, 2 * STATENO);
  gsl_matrix *jacobian = &dfdy_mat.matrix;
  gsl_matrix *jacobian_deriv = gsl_matrix_alloc (2 * STATENO, 2 * STATENO);
  ...
  gsl_matrix_view submatrix1 =
    gsl_matrix_submatrix (jacobian, 0, STATENO, STATENO, STATENO);
  gsl_matrix *upperright_submatrix = &submatrix1.matrix;

  gsl_matrix_view submatrix2 =
    gsl_matrix_submatrix (jacobian, STATENO, 0, STATENO, STATENO);
  gsl_matrix *lowerleft_submatrix = &submatrix2.matrix;

  /* .. and it's derivative */
  gsl_matrix_view submatrix1_deriv =
    gsl_matrix_submatrix (jacobian_deriv, 0, STATENO, STATENO, STATENO);
  gsl_matrix *upperright_submatrix_deriv = &submatrix1_deriv.matrix;

  gsl_matrix_view submatrix2_deriv =
    gsl_matrix_submatrix (jacobian_deriv, STATENO, 0, STATENO, STATENO);
   ...
   *The jacobian is of column size 2*STATENO. Therefore the submatrices (hamiltonian) *
   * are of column size STATENO                                                                                      */
  ...
  for (nt = 0; nt < 2 * STATENO; nt++)
    {
      dfdt[nt] = 0.0;
      for (mt = 0; mt < 2 * STATENO; mt++)
	{
...
integrate (double *input, double initial, double final, void *param)
{
  ...
  gsl_odeiv_step *s = gsl_odeiv_step_alloc (T, 2 * STATENO);
  gsl_odeiv_control *c =
    gsl_odeiv_control_standard_new (ABSERROR, RELERROR, YERROR, YPRIMEERROR);
  gsl_odeiv_evolve *e = gsl_odeiv_evolve_alloc (2 * STATENO);
  gsl_odeiv_system sys = { func, jac, 2 * STATENO, param };
  dt = DT;
  t = initial;
  while (t < final)
    {
