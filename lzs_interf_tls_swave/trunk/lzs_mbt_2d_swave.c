...
main (int argc, char **argv)
{
  ...
  GArray *time_magsq = g_array_new (FALSE, FALSE, sizeof (double));
  GArray *time_fermimag = g_array_new (FALSE, FALSE, sizeof (double));
  ...
  FILE *tmagsqfile = fopen (argv[argv_count++], "w");
  FILE *tfermimagfile = fopen (argv[argv_count++], "w");
  ...
  double probloc, prob, probavg, probavgsq;
  ...
  long fermicount_loc, fermicount;
  double fermimag_loc,fermimag;
  ...
  for (t = t_init; t <= t_final; t = t + t_inc)
	...
	{			//kx is being parallelized
	  ...
	  for (ky = kyinit; ky <= kyfinal; ky = ky + kyinc)
	      ...
	      if (result == GSL_FAILURE)
		{
	      ...
	      if(fabs(fk)<=fabs(params.muamp)/10.0) //Very close to fermi surface
		{
		  fermicount_loc++;
		  fermimag_loc = fermimag_loc + phi[1] * phi[1] + phi[3] * phi[3];
		}//Append defect density at Fermi surface
      ...
      MPI_Reduce (&probcount_loc, &probcount, 1, MPI_LONG, MPI_SUM, 0,
		  MPI_COMM_WORLD);
      MPI_Reduce (&fermicount_loc, &fermicount, 1, MPI_LONG, MPI_SUM, 0,
		  MPI_COMM_WORLD);
      MPI_Reduce (&fermimag_loc, &fermimag, 1, MPI_DOUBLE, MPI_SUM, 0,
		  MPI_COMM_WORLD);
      ...
      if (pid == 0)
	{
	  ...
	  g_array_append_val (time_magsq,t);
	  g_array_append_val (time_fermimag,t);
	  ...
	  fermimag = fermimag/fermicount;
	  fermimag = 2.0 * fermimag -1.0;
	  g_array_append_val (time_fermimag, fermimag);

	  //Calculate magnetizations
	  adbmag_global = 2.0 * adbmag_global -1.0;
	  adbmag_global = -adbmag_global;
	  probavg = 2.0 * probavg - 1.0;
	  probavgsq = probavg * probavg;
	  ...
	  g_array_append_val (time_adbmag,adbmag_global);
  	  g_array_append_val (time_magsq, probavgsq);
	}
	  ...
	  if (pid == 0)
	    {
	      dump_out_strobe (scaled_time, probavg, fidelity_global,
			       deltaglob_abs, defect_density_global,fermimag,
  ...
  if (pid == 0)
    {
      ...
      dump_out_probs (time_magsq, tmagsqfile, &params);
      dump_out_probs (time_fermimag,tfermimagfile,&params);
      //tau = compute_nonzero (time_fidelity_deriv_mag,&params);
      //dump (reltime,tau);

      qf.probavg = get_qfactor (time_magnetization);
      qf.probavgsq = get_qfactor(time_magsq);
      ...
      qf.fermimagavg = get_qfactor (time_fermimag);
  ...
  g_array_free (time_magsq, TRUE);
  g_array_free (time_fermimag, TRUE);
