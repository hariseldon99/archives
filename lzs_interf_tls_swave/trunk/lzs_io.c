...
dump_out_strobe (double scaled_time, double mag, double fidelity,
		 double delta, double defect_density, double fermimag, double resen,
		 double fidsus)
{
  printf
    ("\n\n------------------------------------------------------------------------------------------------------------------------------------------");
  ...
  printf
    ("\n--------------------------------------------------------------------------------------------------------------------------------------------");
  printf
    ("\n magnetization_s   |     fidelity_s   | absolute_delta_s | defect_density_total_s | magnetization_fermi_s  |residual_energy_s |    fid_suscp_s   |");
  printf
    ("\n---------------------------------------------------------------------------------------------------------------------------------------------");
  printf
    ("\n %-3.14lf | %-3.14lf | %-3.14lf | %-3.14lf        | %-3.14lf        | %-3.14lf | %-3.14lf |",
     mag, fidelity, delta, defect_density, fermimag, resen, fidsus);
  printf
    ("\n---------------------------------------------------------------------------------------------------------------------------------------------");
}
...
final_out_mbt (time_t begin, time_t end, double gridsize, void *qfacts)
{
  ...
  printf
    ("\n-------------------------------------------------------------------------------------------------------------------------------------------------");
  printf
    ("\n magnetization   |     mag sq     |    fidelity    | absolute delta | defect  density (total)| magnetization  (fermi only) |residual  energy");  printf

    ("\n-------------------------------------------------------------------------------------------------------------------------------------------------");
  printf ("\n %-3.12lf | %-3.12lf | %-3.12lf | %-3.12lf | %-3.21lf | %-3.21lf     | %-3.12lf |",
	  probavg, probavgsq,fidavg, deltavg, ndavg, fermimagavg ,ekavg);
  printf
    ("\n-------------------------------------------------------------------------------------------------------------------------------------------------");
  printf ("\n\nRuntime metrics :");
  printf ("\n----------------------------------");
