...
/*Factor (usually unity) that boosts the pump pulse up a bit if needed*/
#define ETA 1.0
...
#define DIM 25
/*Output File(s)*/
char OUT_FILE_QUASI[] = "result/floquet_smallnos6_e_115.dat";
...
main (void)
{
  ...
  double eni, enj;
  dcmplx floquet4essl[DIM][DIM], floqueteigenvalues[DIM], dummy[3];

  FILE *floqueteigenval;
  ...
  for (tfix = TFIXMIN; tfix <= TFIXMAX; tfix = tfix + TFIXINC)
    {
      ...
      epsilonpump = ETA * D24 * amp (tfix, pumptime);
...
#pragma omp for schedule(dynamic,chunk)
      ...
      for (i = 0; i < DIM; i++)
	{
      ...
      zgeev (0, floquet4essl, DIM, floqueteigenvalues, dummy, 2, select, DIM,
	     aux, naux);
      /*End Diagonalization */
