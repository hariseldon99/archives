...
main (int argc, char **argv)
{
  ...
  PetscBool leftavg = PETSC_FALSE;
  PetscReal delta_e = DEFAULTSHELL;
  PetscReal e_init = INITENERGYDEFAULT;
  int mpisize, mpirank;
  ...
  /*
     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
     Read e_init = <\psi(0)|H|\psi(0)> from input
     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - 
   */

  ierr = PetscOptionsGetReal (NULL, "-init_e", &e_init, NULL);
  CHKERRQ (ierr);

  ierr = PetscOptionsGetReal (NULL, "-delta_e", &delta_e, NULL);
  CHKERRQ (ierr);

  ierr = PetscOptionsGetBool (NULL, "-draw_out", &willdraw, NULL);
  CHKERRQ (ierr);

  ierr = PetscOptionsGetBool (NULL, "-left_avg", &leftavg, NULL);
  CHKERRQ (ierr);
  ...
  ierr =
    PetscPrintf (PETSC_COMM_WORLD,
		 " Energy of initial state = %F ...\n", e_init);
  CHKERRQ (ierr);

  /*
     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
     Select all eigenstates whose energies lie within delta_e of e_init
     Either left interval (left avg) or the default right interval (right avg)
     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   */

  //Scatter evals to all processors
  Vec evals_seq;
  VecScatter evals_ctx;		//Scatter context
  ierr = VecScatterCreateToAll (evals, &evals_ctx, &evals_seq);
  CHKERRQ (ierr);
  ierr =
    VecScatterBegin (evals_ctx, evals, evals_seq, INSERT_VALUES,
		     SCATTER_FORWARD);
  CHKERRQ (ierr);
  ierr =
    VecScatterEnd (evals_ctx, evals, evals_seq, INSERT_VALUES,
		   SCATTER_FORWARD);
  CHKERRQ (ierr);

  PetscInt evals_stride, evals_gotsize;
  PetscScalar evals_i;
  PetscReal evals_ireal, ediff;

  ierr = VecGetSize (evals_seq, &evals_gotsize);
  CHKERRQ (ierr);

  //Count the number of eigenstates whose energies lie within delta_e of e_init
  PetscInt chosen_evals_size = 0;
  for (evals_stride = 0; evals_stride < evals_gotsize; evals_stride++)
    {
      ierr = VecGetValues (evals_seq, 1, &evals_stride, &evals_i);
      CHKERRQ (ierr);
#if defined(PETSC_USE_COMPLEX)
      evals_ireal = PetscRealPart (evals_i);
#else
      evals_ireal = evals_i;
#endif
      if (leftavg)
	{
	  ediff = e_init - delta_e;
	  if ((evals_ireal <= e_init) && (evals_ireal >= ediff))
	    chosen_evals_size++;
	}
      else
	{
	  ediff = e_init = delta_e;
	  if ((evals_ireal <= ediff) && (evals_ireal >= e_init))
	    chosen_evals_size++;
	}
    }

  PetscInt detected_dim;
  ierr = VecGetSize (evals_seq, &detected_dim);
  CHKERRQ (ierr);
  if (chosen_evals_size == 0)
    SETERRQ (PETSC_COMM_WORLD, ierr,
	     "\n No mucan states detected. Adjust delta_e\n");
  else
    ierr =
      PetscPrintf (PETSC_COMM_WORLD,
		   " # of mucan states detected = %d out of %d ... ",
		   chosen_evals_size, detected_dim);
  CHKERRQ (ierr);

  //Allocate an array to locally store these component indices
  PetscInt *ids_loc = malloc (chosen_evals_size * sizeof (PetscInt));
  ...
  /*
     ...
     \sum_\alpha (b^\dagger_i b_j )_{\alpha\alpha}
     - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
   */
  ...
  for (sitestride1 = Istart; sitestride1 < Iend; sitestride1++)
      ...
      for (sitestride2 = 0; sitestride2 < matsize; sitestride2++)
	{
	  ...
	  if ((is_forbidden (sitestride1_vec, 1, latsize) == GSL_SUCCESS)
	    ...
	    {
	      element = 0.0;
	      for (count = 0; count < chosen_evals_size; count++)
		{
		  alpha = ids_loc[count];
		  ...
		  for (mu = 0; mu < dim; mu++)
		      ...
		      for (lambda = 0; lambda < dim; lambda++)
			{
			  ...
			  element = element + U_al * U_am * value;
			}
		    }
		  ierr = MatRestoreRow (U_seq, alpha, NULL, NULL, &u_row_arr);
		  CHKERRQ (ierr);
		}
	      //Normalize with number of chosen states
	      element = element / chosen_evals_size;
  ...
  CHKERRQ (ierr);
  ierr = MatSetSizes (Nk, PETSC_DECIDE, PETSC_DECIDE, matsize, matsize);
  CHKERRQ (ierr);
  ...
  ierr =
    PetscPrintf (PETSC_COMM_WORLD,
		 "\n Computed ensemble average in %f secs\n",
		 point2 - point1);
  CHKERRQ (ierr);
  /**************************** End Ensemble Average*******************************/
  ...
  if (willdraw)
	...
	PetscViewerDrawOpen (PETSC_COMM_WORLD, 0,
      ...
      ierr =
	PetscViewerDrawOpen (PETSC_COMM_WORLD, 0,
			     "Hopping Matrix Ensemble Avg", PETSC_DECIDE,
			     PETSC_DECIDE, PETSC_DECIDE, PETSC_DECIDE,
			     &dekhbo);
  ...
  ierr =
    PetscPrintf (PETSC_COMM_WORLD,
		 ...
		 filenames_eth_bin_eavg[0]);
  CHKERRQ (ierr);
  ierr =
    PetscViewerBinaryOpen (PETSC_COMM_WORLD, filenames_eth_bin_eavg[0],
  ...
  ierr =
    PetscPrintf (PETSC_COMM_WORLD,
		 ...
		 filenames_eth_bin_eavg[1]);
  CHKERRQ (ierr);
  ierr =
    PetscViewerBinaryOpen (PETSC_COMM_WORLD, filenames_eth_bin_eavg[1],
			   FILE_MODE_WRITE, &outview);
  CHKERRQ (ierr);
  ...
  /*******************************Free Work Space*********************************/
  ierr = VecScatterDestroy (&evals_ctx);
  CHKERRQ (ierr);
  ierr = VecDestroy (&evals_seq);
  CHKERRQ (ierr);
